...
var _node_two_captcha = _interopRequireDefault(require("@infosimples/node_two_captcha"));

var _firebaseInit = _interopRequireDefault(require("./firebaseInit"));
...
const storage = (0, _storage.getStorage)(); // Declare your client

const client = new _node_two_captcha.default('c41a16e62a22835d699e613ce5beb246', {
  timeout: 60000,
  polling: 5000,
  throwErrors: false
});
...
async function getResultsByZipCode(zip) {
  ...
  await page.goto(link); // wait for input field selector to render

  await page.waitForSelector('#searchText');
  await page.click('#searchText');
  await page.keyboard.type(zip); // press enter on your keyboard
  ...
  await traverseZipPage(page); // await page.close();

  return; // bye

  const names = await page.$$(".name"); // const page2 = await browser.newPage();
...
async function getIndividualPageData(page) {
  ...
  _values.employment = _htmlTableToJson.default.parse(employment).results;
...
async function solveCaptcha(page, captchaImageID, captchaTextboxID) {
  ...
  const captchaRef = (0, _storage.ref)(storage, file); // 'file' comes from the Blob or File API

  await (0, _storage.uploadBytes)(captchaRef, data).catch(error => {
    console.log("error on uploading captcha", error);
  });
  const url = await (0, _storage.getDownloadURL)(captchaRef);
...
function today() {
  ...
  let year = date_ob.getFullYear(); // prints date & time in YYYY-MM-DD format
